...
***********************************************************************************************************************/
  ...
  UINT8 tss_u8SampleIntFlag;
  UINT8 tss_u8HWTimerFlag = 0u;
  ...
  ****************************************************************************/
  ...
  void TSS_NoiseAmplitudeFilterInit(void) 
  {
      ...
      {
        ((UINT16 *) tss_acp8ElectrodeRAMData[u8counter])[0] = 0u; /* Set default value */
      }
  }
  #endif
  ...
  ****************************************************************************/
  UINT8 UNI_SampleElectrode(UINT8 u8ElecNum, UINT8 u8Command)
  {
      ...
      UINT8 u8FaultCnt;
        ...
        UINT16 *u16AmplitudeFilterBaseTemp;        
      #endif
      ...
      {  
        ...
        #if TSS_USE_NOISE_AMPLITUDE_FILTER
          u16AmplitudeFilterBaseTemp = &(((UINT16 *) tss_acp8ElectrodeRAMData[u8ElecNum])[0]);
          
          if (*u16AmplitudeFilterBaseTemp == 0u) {
            i8AmplitudeFilterDeltaLimitTemp = 127;
          } else {
            i8AmplitudeFilterDeltaLimitTemp = (INT8) (tss_acp8ElectrodeROMData[u8ElecNum])[0];            
          }
        #endif
        ...
        while(u8NSamp)
        {
            ...
            {            
                ...
                {
                  #if TSS_USE_NOISE_AMPLITUDE_FILTER
                    if (TSS_u16NoiseAmplitudeFilter(u16CapSubSample, u16AmplitudeFilterBaseTemp, i8AmplitudeFilterDeltaLimitTemp)) 
                    {
                      u8FaultCnt++;                                      
                    } 
                    else 
        ...
        if ((tss_u16CapSample < TSS_KEYDETECT_CAP_LOWER_LIMIT) && (u8ElectrodeStatus != TSS_SAMPLE_ERROR_CHARGE_TIMEOUT)) 
        {
          ...
          if (u8ElectrodeStatus == TSS_SAMPLE_STATUS_OK) {
            if (*u16AmplitudeFilterBaseTemp == 0u) {
              *u16AmplitudeFilterBaseTemp = (UINT16) (tss_u16CapSample / tss_CSSys.NSamples);
            }
          } else {
            *u16AmplitudeFilterBaseTemp = 0u; /* Initiate Amplitude Filter Base refresh */
          }
        #endif
  ...
  ****************************************************************************/
  ...
  UINT8 TSS_SensorInit(UINT8 u8Command)
  {
      ...
      for(u8ElCounter1 = 0; u8ElCounter1 < TSS_N_ELECTRODES; u8ElCounter1++)
      {
        ...
        {  
          psMethodROMDataStruct = (UINT32 **) ((UINT32 *)tss_acp8ElectrodeROMData[u8ElCounter1])[0];
          /* Execute MethodControl Function*/
          u8Result |= ((TSS_METHOD_CONTROL) psMethodROMDataStruct[0]) (u8ElCounter1, u8Command);           
          /* Check if another the same modules are there */
          for(u8ElCounter2 = u8ElCounter1; u8ElCounter2 < TSS_N_ELECTRODES; u8ElCounter2++)
          {
            if (psMethodROMDataStruct == (UINT32 **) ((UINT32 *)tss_acp8ElectrodeROMData[u8ElCounter2])[0]) 
            {
  ...
  ****************************************************************************/
    ...
    {
      /* Clear HW Timer Flag */
      ((FTM_MemMapPtr) tss_psElectrodeTimertAdr)->SC &= TSS_HW_TIMER_FLAG_ACK;         /* Clear HW TImer Flag */
      /* Set Overflow Flag */
      tss_u8HWTimerFlag = 1u;                /* Set measurement Overflow Flag variable */
    }
  #endif
