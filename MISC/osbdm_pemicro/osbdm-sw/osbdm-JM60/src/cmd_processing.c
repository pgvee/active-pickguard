...
void serial_command_send_return(unsigned char rval, unsigned char count){
	
	//KACK what if already sending a command? Cancel old and send new.
	EP4_Buffer[0] = rval;
	EP4_Buffer[1] = count;
	EndPoint_IN(EP4,count);
}
