...
int internal_usb_find_devices(int interfacenum, int the_VID, int the_PID1, int the_PID2) {
	...
	for (libusb_bus = usb_get_busses(); libusb_bus; libusb_bus = libusb_bus->next) {		/* scan through all busses */
    	for (libusb_dev = libusb_bus->devices; libusb_dev; libusb_dev = libusb_dev->next) {	/* scan through all devices */
  			if ((libusb_dev->descriptor.idVendor==the_VID)
				&&((libusb_dev->descriptor.idProduct==the_PID1)||(libusb_dev->descriptor.idProduct==the_PID2))
...
unsigned char internal_usb_open(unsigned int device_no, unsigned int interface_no) {
	...
	if (usb_set_configuration(osbdm_devh,1)) return(1);  // opensourcebdm has only one valid configuration 
	//print("USB Configuration set\r\n");
	if (usb_claim_interface(osbdm_devh,interface_no)) return(1);
...
int osbdm_usb_find_devices(void) {
	return internal_usb_find_devices(0,opensourcebdm_VID,opensourcebdm_PID1,opensourcebdm_PID2);  // Debug port is Interface 0
}

int virtual_serial_usb_find_devices(void) {
	return internal_usb_find_devices(1,opensourcebdm_VID,opensourcebdm_PID1,opensourcebdm_PID1);  // Virtual Serial Port is Interface 1
}

int bootloader_usb_find_devices(void) {
	return internal_usb_find_devices(0,opensourcebdm_VID,opensourcebdm_bootloader_PID,opensourcebdm_bootloader_PID);  // Virtual Serial Port is Interface 1
}
