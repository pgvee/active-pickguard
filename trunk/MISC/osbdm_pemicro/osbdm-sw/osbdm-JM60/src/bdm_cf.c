   ...
   switch (hold_temp_result & 0x0000FFFF) 
   {
    ...
    default:
      ...
      { // still an illegal command error 
        ...
        {
          outp(((byte)cfinp() | 0x02) & 0xFE); // make sure clk high data low
          delay_492ns(2000); // delay 1ms
          outp((byte)cfinp() & 0xFC); // make sure clk low data low
          delay_492ns(2000); // delay 1ms
          hold_temp_result = t_swap17((BDMCF_CMD_NOP | hold_port_state),&hold_swap_results[0] );
...
switch (sub_cmd_num)
        ...
        */
        ...
        case 0x11: // do read sequence - 16 bit opcode - 32 bit address - 
          ...
          else 
          {
            ...
            rxBuff[5] = 0;
            rxBuff[3] = rxBuff[0];
            //rxBuff[3] = 0x01; /* Bus error occured */
          }
        ...
        *pOutputLength = 2;
        ...
        pOutputBuffer[recv_index+4] = 0;
        pOutputBuffer[recv_index+5] = 0; 
