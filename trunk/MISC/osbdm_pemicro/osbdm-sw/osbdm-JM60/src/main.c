...
//  ConfigureUSBDevice();
	...
	for(;;){						// Main Loop
		...
		if(serial_cmd_pending){				// if BDM command is ready, 
			serial_command_exec();			// process BDM command
			serial_cmd_pending = 0x00;		// clear pending flag
			// KACK need to re-enable endpoint 3
		}
		
  	if (sci_virtual_serial_port_is_enabled==TRUE) 
       serial_send_data_if_pending();
